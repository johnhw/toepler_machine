...
// main axle (hex keyed)
color("Green")
translate([-base_l/2+tol-pulley_thick*2,0,axle_height])
{
    rotate([90,0,90])
    {
...
// axle bearings
translate([-base_l/2+support_thick+tol-bearing_thick,0,axle_height])
rotate([0,90,0])
std_bearing();

translate([base_l/2-support_thick-tol,0,axle_height])
rotate([0,90,0])
std_bearing();

translate([-base_l/2+support_thick+tol-bearing_thick,0,handle_h])
rotate([0,90,0])
std_bearing();
